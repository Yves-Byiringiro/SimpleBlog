...
def article_list(request, template_name='article/article_list.html'):
...
def article_detail(request, pk, template_name='article/article_detail.html'):
...
@login_required
def article_create(request, template_name='article/article_form.html'):
...
@login_required
def article_update(request, pk, template_name='article/article_form.html'):
    # article= get_object_or_404(Article, pk=pk)
...
@login_required
def article_delete(request, pk, template_name='article/article_confirm_delete.html'):
    article= get_object_or_404(Article, pk=pk)    
    if request.method=='POST':
        article.delete()
        return redirect('article_list')
