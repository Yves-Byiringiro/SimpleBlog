...
from article.models import Article, Comment, College
...
def cst(request):
    articles_cst = Article.objects.filter(college__name='College of Science and Technology').order_by('-id')
    return render(request,'home/cst.html',{'articles_cst':articles_cst})

def cbe(request):
    articles_cbe = Article.objects.filter(college__name='College of Business and Economics').order_by('-id')
    return render(request,'home/cbe.html',{'articles_cbe':articles_cbe})

def cmhs(request):
    articles_cmhs = Article.objects.filter(college__name='College of Medecine and Health Sciences').order_by('-id')
    return render(request,'home/cmhs.html',{'articles_cmhs':articles_cmhs})

def ce(request):
    articles_ce = Article.objects.filter(college__name='College of Education').order_by('-id')
    return render(request,'home/ce.html',{'articles_ce':articles_ce})

def cass(request):
    articles_cass = Article.objects.filter(college__name='College of Arts and Social Sciences').order_by('-id')
    return render(request,'home/cass.html',{'articles_cass':articles_cass})

def cavm(request):
    articles_cavm = Article.objects.filter(college__name='College of Agriculture, Animal Sciences and Veterinary Medecine').order_by('-id')
